...
const EditAccountForm = ({firstName, lastName, phone, email, formatedPhone, businessName, update})=>{
    ...
    },[firstName, lastName, phone, email, formatedPhone])
    ...
    const validation = (first, last, phone, email)=>{
        ...
        //remove non-numbers from phone number
        let phoneValidation = phone.replace(/[^0-9]+/g, '');

        //validate info w/ the following criteria
        if(!emailValidation || phoneValidation.length !== 10 || firstName.length <= 0 || lastName.length <= 0){
            ...
            if(phoneValidation.length !== 10){
                errorMessages[2].innerHTML = "Input a Valid Phone Number"
                errorIcons[2].style.opacity = 1; 
                document.getElementById("personalPhone").style.border = "1px solid red"; 
            }
    ...
    const handleSubmit = (e)=>{
        ...
        let _phoneNumber = phoneNum.trim() === "" ? phone : phoneNum.trim();
        let _eMail = eMail.trim() === "" ? email : eMail.trim();
        //then pass values to the validation function
        validation(_first, _last, _phoneNumber, _eMail);
    };
